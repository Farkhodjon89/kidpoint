...
const status = [
  {
    title: "Принят",
    text:
      "Ваш заказ принят! Наши менеджеры свяжутся с вами для уточнения всех деталей. ",
  },
  {
    title: "Обработка",
...
const AccountOrder = ({ coupons,openNavigation,setOpenNavigation }) => {
    ...
    const formattedOrders = response.data.orders.map((order) => ({
      ...
      content: {
        ...
        products: order.lineItems.map((item) => ({
          ...
          size: item.size ? item.size[0].value : '',
          color: item.color ? item.color[0].value : '',
          price: `${item.total * item.quantity} UZS`,
        })),
    ...
    return (
      <div className={`${s.pageContent} ${!openNavigation ? s.activeMobile : ""}`}>
        ...
        {orders.map(({ id, date, status, content }) =>
          coupons ? (
            <div className={s.order} key={orders.length}>
              <ul className={s.list}>
                <li className={s.item}>
